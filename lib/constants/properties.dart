// ignore_for_file: constant_identifier_names

class Properties {
  Properties._();

  //General
  static const int delayTimeInSecond = 2;
  static const int delayTimeInMiliSecond = 2000;

  /// return 500 mili second
  static const int animatedTimeInMiliSecond = 500;

  /// return normal blur = 15
  static const double blur_glass_morphism = 15;

  /// return normal opacity = 0.2
  static const double opacity_glass_morphism = 0.2;

  /// return heavily blur = 20
  static const double hevily_blur_glass_morphism = 20;

  /// return heavily opacity = 0.3
  static const double hevily_opacity_glass_morphism = 0.3;

  /// return heavily opacity = 0.3
  static const double opacity_mode = 0.7;

  /// return medium blur = 10
  static const double medium_blur_glass_morphism = 5;

  /// return medium opacity = 0.2
  static const double medium_opacity_glass_morphism = 0.1;

  /// return lightly blur = 5
  static const double lightly_blur_glass_morphism = 5;

  /// return lightly opacity = 0.1
  static const double lightly_opacity_glass_morphism = 0.1;
}

// static field Singleton